...
Fixed::
* Fix renewal notification scheduling on iOS 27.
====
...
Fixed::
* Fix for older LV (SeID) Idemia cards on Android and iOS NFC libs.
* Fix iOS uses "too much" data (left over files will be removed with next version)
* Fix retries when network issues occur during registration.
* Fix ABIV face recognition step showing English instead of the app language
* Make hybrid app compatible with Chrome 51 (Android 7.0)
* Visual improvements to new registration method selection.
* Fix country select animation.
* Fix continuous menu flickering when pressing "Upgrade Smart-ID level" button.

Security::
* Updated 3rd party libraries, inc ReadID M128
====
...
====
Added::
* Add brokered RP name and redesign transaction view.
* Added ID card libs with LV support

Changed::
* Regroup registration methods
* New design for country selection
* Redesign the How-to-use tutorial with method-based slides
* Leaving only the Artea logo
* Integrate LIB v25.3 (Android)
* Integrate LIB v25.5 (iOS)

Security::
* Updated 3rd party libraries, inc ReadID M127
====
...
====
Fixed::
* "Confirm" button is not working during registration (iOS)
* Voiceover/keyboard support is gone (iOS)
====
...
====
Added::
* Add iOS lock screen widget for QR scanner

Fixed::
* Disable Impeller render engine for Android 12 and lower (fix for CAT devices)

Changed::
* Add new error code for ID-card auth token validation failure and handle it as critical error
* Change timeout modal to be less intrusive
* Add iOS nfc lib version and license info
====
...
====
Fixed::
* Update nfc lib to work with iOS 26.4
====
...
====
Fixed::
* Remove id-card NFC signing full screen loader success.
* Improve CAN field validation UX
* Don't show success animation in case of web2app or app2app flow.
* App does not open push transaction when pending QR transaction in background
* Add id card NFC dependencies license info and dependency documentation
* Better error handling when PIN1 or PIN2 is locked for device link operations.

Changed::
* Integrate LIB v25.2 (Android)
* Integrate LIB v25.4 (iOS)

Security::
* Updated 3rd party libraries, inc ReadID M124
====
...
====
Fixed::
* Fix registration loader regression.
* Fix redirect issue to Scan QR code view from widget if app cold starts
* Show better error message when starting device link transaction without active account.
* Fix OK button active state when PIN confirmation is required.
* Fix RP buttons disabled state during loading.
* Fix youtube configuration issue error if trying to watch embed videos
* Sync registration steps animation with screen transitions.
* Fix success loader checkmark not showing when animation to next screen.
* Fix infinite loader when inactive account (e.g. deleted in portal) is deleted in app.

Changed::
* Always show confirm transaction modal when transaction source is QR code.
* Add RP name to displayText and VC modal.
* Add instructions modal before starting liveness check during registration.
* Increase iOS deployment target version to  iOS 15

Added::
* Add Smart-ID+ introduction for end-user
* Added support for registering using ID-Card (EE) with NFC 

Security::
* Updated 3rd party libraries, inc ReadID M123
====
